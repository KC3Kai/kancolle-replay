...
function FLEET(){
	...
	this.addFleet = function(fleet, hp) {
	...
	this.addCombinedFleet = function(main, escort, hp, ehp) {
		var shipList = [];
		for (var i = 0; i < fleet.length; i++) {
			this.mainFleet.push({
	...
	this.addSupport = function(fleet, boss) {
		
	}
	
	this.formatFleet = function(fleet) {
		var ships = "";
...
*/
	...
	processSortie = function(){
		
	}
}

PLAYERTEXT.prototype.processText = function() {
	var world = API.world;
	var map = API.mapnum;
	var combined = API.combined
	var startData = API.battles[0].data;

	if(combined) player.addCombinedFleet(API.fleet1,API.fleet2,startData.api_nowhps.slice(1,7), startData.api_nowhps_combined.slice(1,7));
	else player.addFleet(API['fleet'+API.fleetnum],startData.api_nowhps.slice(1,7));
	if(API.support1 > 0) player.addSupport(API['fleet'+API.support1]);
	if(API.support2 > 0) player.addSupport(API['fleet'+API.support2], true);
	...
	/*else {
		...
		getTextRow("SORTIE_START",[world, map, new Date(API.time*1000)]));

		if(!combined) {
			getTextRow("FLEET_COMPOSITION",[getFleet(API['fleet'+API.fleetnum])]));
		} else {
			getTextRow("COMBINDED_FLEET_COMPOSITION",[getFleet(API.fleet1), composeFleet(API.fleet2)]));
		}

		if(API.support1 > 0) {
			getTextRow("SUPPORT_FLEET_COMPOSITION", [getFleet(API['fleet'+API.support1]), 1]));
		}

		if(API.support2 > 0) {
			getTextRow("SUPPORT_FLEET_COMPOSITION", [getFleet(API['fleet'+API.support2]), 2]));
		}
	}*/
}
