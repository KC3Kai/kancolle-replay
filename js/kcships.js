...
Fleet.prototype.getTransport602 = function() {
	let tpBase = this.getTransport();
	let tpBonus = 0;
	let bonusById = { 166: 9, 167: 8, 230: 41, 355: 19, 436: 16, 449: 35, 482: 22, 494: 17, 495: 23, 496: 15, 497: 17, 498: 23, 499: 38, 514: 27, 525: 5, 526: 7 };
	for (let ship of this.ships) {
		if (ship.HP <= 0 || ship.retreated) continue;
		if (ship.mid == 487) {
			tpBonus += 8;
...
Fleet.prototype.getTransport612 = function() {
	let tpBase = this.getTransport();
	let tpBonus = 0;
	let bonusById = { 166: 8, 167: 11, 230: 13, 355: 14, 436: 10, 449: 15, 482: 13, 494: 11, 495: 13, 496: 5, 497: 7, 498: 9, 499: 14, 514: 17, 525: 10, 526: 12, 576: 18 };
	for (let ship of this.ships) {
		if (ship.HP <= 0 || ship.retreated) continue;
		if (ship.mid == 487) {
			tpBonus += 8;
	...
	return Math.floor(tpBonus + .75*tpBase);
}
...
Ship.prototype.loadEquips = function(equips,levels,profs,addstats,isSupport) {
	...
	for (var i=0; i<equips.length; i++){
		...
		if (eq.mid == 576) installeqs.R35 = installeqs.R35 + 1 || 1;
	...
	if (installeqs.panzer) {
		installModAll *= 1.15;
		this.installFlat *= 1.15;
		this.installFlat += 15;
	}
	if (installeqs.R35) {
		installModAll *= 1.2;
	...
	if ((installeqs.soukoutei == 1 || installeqs.armedDaihatsu == 1) && (installeqs.soukoutei || 0) < 2 && (installeqs.armedDaihatsu || 0) < 2) {
		let numA = this.equips.filter(eq => [68,166,193,436,449,514,525,526,576].includes(eq.mid)).length;
	...
	if (MECHANICS.installRevamp) {
		...
		if (installeqs.R35) this.softSkinMult *= 1.5;
	...
	if (MECHANICS.installRevamp) {
		...
		if (installeqs.R35) this.pillboxMult *= 1.5;
	...
	if (MECHANICS.installRevamp) {
		...
		if (installeqs.R35) this.isoMult *= 1.2;
	...
	if (MECHANICS.installRevamp) {
		...
		if (installeqs.R35) this.harbourSummerMult *= 1.6;
	...
	if (MECHANICS.installRevamp) {
		...
		if (installeqs.R35) this.supplyPostMult *= 1.3 * installbonus1;
	...
	if (installeqs.R35) this.anchoragePostMult *= 1.2;
	...
	if (installeqs.R35) this.dockPostMult *= 1.15;
	if (installeqs.R35 >= 2) this.dockPostMult *= 1.15;
...
Ship.prototype.NBchance = function() {
	if (this._nbchance === undefined) {
		...
		if (this.equips.find(eq => eq.mid == 129)) this._nbchance += 5;
		if (this.LUK >= 50) this._nbchance += 65 + Math.floor(Math.sqrt(this.LUK-50)) + Math.floor(Math.sqrt(this.LVL)*.8);
...
CV.prototype.NBtypes = function() {
	if (MECHANICS.CVCI && this.canNBAirAttack() && this.hasNBAirGear()) {
		let nbtypes = [];
		let numFuze = 0, numF = 0, numA = 0, numB = 0, numB2 = 0;
		for (let i=0; i<this.equips.length; i++) {
			if (this.side == 0 && this.planecount[i] <= 0) continue;
			let eq = this.equips[i];
			if (eq.btype == B_NIGHTFIGHTER) numF++;
			else if (eq.btype == B_NIGHTBOMBER) numA++;
			else if (eq.btype == B_NIGHTBOMBER_DB) numB++;
			else if (eq.btype == B_NIGHTBOMBER2) numB2++;
			if (eq.mid == 320) numFuze++;
		}
		let numNight = numF + numA + numB;
		if (numF >= 2 && numA) nbtypes.push(61);
		if (numF && numA) nbtypes.push(62);
		if (numFuze + numB && numNight + numFuze >= 2) nbtypes.push(63);
		if (numF && numNight + numB2 >= 3) nbtypes.push(64);
		return nbtypes;
	}
	return Ship.prototype.NBtypes.call(this);
}
...
AO.prototype.loadEquips = function(equips,levels,profs,addstats,isSupport) {
	...
	if (this.canAirAttackNB && CV.prototype.hasNBAirGear.call(this)) {
		...
		this.NBtypes = CV.prototype.NBtypes;
